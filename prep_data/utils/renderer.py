...
class OrthoTexturedRenderer(OrthoColoredRenderer, TexturedRenderer):
    ...
    def on_changed(self, which):
        ...
        if 'texture_image' in which or 'ortho' in which:
            ...
            texture_data = np.array(
                self.texture_image * 255., dtype='uint8', order='C')
    ...
    def mesh_tex_coords(self):
        ...
        data = np.asarray(self.vt.r[ftidxs].astype(
            np.float32)[:, 0:2], np.float32, order='C')
...
def _setup_ortho(gl, l, r, b, t, near, far, view_matrix):
    ...
    gl.LoadIdentity()
    # top and bottom switched for opencv coordinate system
    gl.Ortho(l, r, t, b, near, far)
    ...
    view_mtx = np.asarray(
        np.vstack((view_matrix, np.array([0, 0, 0, 1]))), np.float32, order='F')
