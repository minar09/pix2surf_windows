...
class Isomapper():
    def __init__(self, vt, ft, tex_res, bgcolor=np.zeros(3)):
        vt3d = np.dstack((vt[:, 0] - 0.5, 1 - vt[:, 1] -
                          0.5, np.zeros(vt.shape[0])))[0]
        ...
        self.f = ft
        self.rn_tex = OrthoTexturedRenderer(
            v=vt3d, f=ft, ortho=ortho, vc=np.ones_like(vt3d), bgcolor=bgcolor)
    ...
    def render(self, frame, proj_v, f, visible_faces=None, inpaint=True, inpaint_segments=None):
        ...
        if visible_faces is not None:
            ...
            if inpaint:
                visible = cv2.erode(self.rn_vis.r, np.ones(
                    (self.tex_res / 100, self.tex_res / 100)))
                ...
                if inpaint_segments is None:
                    ...
                    for i in range(np.max(inpaint_segments) + 1):
                        ...
                        part = cv2.inpaint(
                            np.uint8(tex * 255), np.uint8((1 - seen) * 255), 3, cv2.INPAINT_TELEA) / 255.
