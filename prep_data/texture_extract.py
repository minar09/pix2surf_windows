...
def main(vt_ft_path, gar_type, mesh_location,  texture_location,
    ...
    if inpaint_mask is not None:
        tex_save = cv2.inpaint(np.uint8(tex_save * 255),
...
if __name__ == "__main__":
    ...
    args.add_argument("--mesh_location", type=str,
                      help="Location of the registered mesh")
    args.add_argument("--texture_location", type=str,
                      help="Location of the textured image")
    args.add_argument(
        "--side", type=str, help="front|back. Whether the texture is to be stored for the front or back")
    args.add_argument("--save_tex_location", type=str,
                      help="Location of the final texture")
    opt = args.parse_args()
    ...
    opt.inpaint_mask = os.path.join(
        TWO_UP, 'assets/inpaint_masks/{}_{}.jpg'.format(opt.gar_type, opt.side))
