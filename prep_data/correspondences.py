...
if __name__ == "__main__":
    ...
    args.add_argument("--sv_cords", type=str,
                      help="Location where coordinates are to be stored")
    args.add_argument(
        "--side", type=str, help="Whether it is the front or the back we are dealing with front | back")
    args.add_argument("--size", type=int, default=256,
                      help="The size of the final image")

    args = args.parse_args()

    from os.path import dirname as up
    TWO_UP = up(up(os.path.abspath(__file__)))
    args.fmap_location = os.path.join(
        TWO_UP, 'assets/fmaps/{}.npy'.format(args.gar_type))
    args.bmap_location = os.path.join(
        TWO_UP, 'assets/bmaps/{}.npy'.format(args.gar_type))
    args.cam_file = os.path.join(TWO_UP, 'assets/cam_file.pkl')

    main(args.mesh_file, args.sv_cords, args.side, args.size,
         args.fmap_location, args.bmap_location, args.cam_file, args.gar_type)
