...
def get_part(front, path_fmap, path_mesh):
    ...
    submesh_verts, submesh_faces, _, v_ids = get_submesh(
        mesh.v, mesh.f, verts_retained=verts)
...
def get_callback(rend, mask, smpl, v_ids_template, v_ids_sides, faces_template, faces_side, display, disp_mesh_side, disp_mesh_whl, mv, mv2, save_dir, show):
    ...
    if display:
        ...
        def cb(_):
            ...
            if save_dir != None:
                cv2.imwrite(save_dir + '/' + str(glob_var) +
                            '.jpg', 255 * (1 - debug))
...
def get_dist_tsfs(mask):
    ...
    dist_i[dist_i < 0] = 0
    dist_i[dist_i > 50] = 50
    # zero where the binary image is 1
    dist_o = cv2.distanceTransform(255 - np.uint8(mask * 255), dst_type, 5)
    dif_mask = mask.copy()
    dif_mask[mask == 1] = 0.5
...
def init_fit(opt, dist_o, dist_i, dif_mask, rn_m, smpl_h,
    ...
    tgt_pose = get_pose_prior(
        init_pose_path=opt.init_pose_path, gar_type=opt.gar_type)
...
def final_fit(
    ...
    import scipy.sparse as sp
    def sparse_solver(A, x): return sp.linalg.cg(A, x, maxiter=500)[0]
    ...
    if opt.ref_wt_coup:
        ...
        E['coupling'] = (
            v + v_offset - smpl_h_ref[v_ids_template]) * opt.ref_wt_coup
...
if __name__ == '__main__':
    ...
    parser.add_argument("--load_from_json", type=str,
                        help="None| location of json file from which arguments are to be loaded")
    # ===================================
    #           FILE LOCATIONS
    # ===================================
    parser.add_argument("--save_file", type=str,
                        help="location of final obj file saved")
    parser.add_argument("--mask_file", type=str,
                        help="location of initial mask file")
    parser.add_argument("--save_opt_images", type=str, default="None",
                        help="Determines whether optimization silhouette images are saved or not. If 'None', then nothing gets saved")
    parser.add_argument("--save_iou_file", type=str, default="None",
                        help="location of the txt file where mean iou is to stored")
    parser.add_argument("--save_json_file", type=str, default="None",
                        help="location of the json file in which all the input arguments are to be stored")
    # ==============================
    #           FLAGS
    # ==============================
    parser.add_argument("--front", type=str2bool, default=True,
                        help="Whether to use the front or the back")
    parser.add_argument("--display", type=str2bool, default=True,
                        help="Whether to display callback or not- global var controls whether callback is none or there")
    parser.add_argument("--show", type=str2bool, default=True,
                        help="Whether or not to display the silhouette matching or not")
    parser.add_argument("--disp_mesh_whl", type=str2bool, default=False,
                        help="Display the whole mesh in debugging as well or not")
    parser.add_argument("--disp_mesh_side", type=str2bool, default=False,
                        help="Display the mesh corresponding to the side while debugging or not")
    parser.add_argument("--gender", type=str, default="neutral",
                        help="male|neutral. If chosen male only the first ten coefficients will be used")
    parser.add_argument("--gar_type", type=str, default="shorts",
                        help="Garment category we are dealing with shorts|pants|shirts")
    # ================================
    #       INIT STAGE FLAGS
    # ================================
    parser.add_argument("--init_first_stage", type=str,
                        default="Pose", help="Pose|Shape|Trans|Both")
    parser.add_argument("--init_second_stage", type=str,
                        default="Both", help="Pose|Shape|Both|None all as a string")
    parser.add_argument("--init_pose_joints", type=str, default="1,2",
                        help="list of joints used for optimization in the first stage")
    parser.add_argument("--init_fst_wt_betas", type=float, default=4,
                        help="Initialization first phase weight of shape prior")
    parser.add_argument("--init_fst_wt_pose", type=float, default=60,
                        help="Initialization first phase weight of pose prior")

    parser.add_argument("--init_sec_wt_betas", type=float, default=8,
                        help="Initialization second phase weight of shape prior")
    parser.add_argument("--init_sec_wt_pose", type=float, default=40,
                        help="Initialization second phase weight of pose prior")
    parser.add_argument("--init_fst_wt_dist_o", type=float, default=2,
                        help="Initialization first phase, weight of the dist_o term")
    parser.add_argument("--init_fst_wt_mask", type=float, default=100,
                        help="Initialization first phase, weight of the mask term")
    parser.add_argument("--init_sec_wt_dist_o", type=float, default=4,
                        help="Initialization second phase, weight of the dist_o term")
    parser.add_argument("--init_sec_wt_mask", type=float, default=100,
                        help="Initialization second phase, weight of the mask term")
    # ===================================
    #       REFINEMENT STAGE FLAGS
    # ===================================
    parser.add_argument("--ref_wt_dist_o", type=float, default=200,
                        help="Weight of the dist_o term in the refinement")
    parser.add_argument("--ref_wt_mask", type=float, default=100,
                        help="weight of the mask term in the refinement")
    parser.add_argument("--ref_wt_coup", default=0,
                        type=float, help="Weight of coupling term")
    parser.add_argument("--ref_wt_proj", default=0.1,
                        type=float, help="Weight of the projection term")
    parser.add_argument("--ref_wt_bd", default=4, type=float,
                        help="weight of the boundary smoothing term")
    parser.add_argument("--ref_wt_lap", default=1000, type=float,
                        help="weight of the laplacian term in the refinement step")
    parser.add_argument("--ref_joint_list_coup", type=str,
                        default="1,2", help="optimize pose during the refinement")
    parser.add_argument("--ref_wt_shp", type=float, default=0,
                        help="Whether to use shape regularization in the refinement term")
    parser.add_argument("--ref_wt_pose", type=float, default=0,
                        help="Whether to use pose prior in the final refinement term")
    parser.add_argument("--ref_use_betas", type=str2bool, default=False,
                        help="Whether to use betas as variables in the refinement")
