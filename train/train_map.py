...
class MapNet(ABC):
    ...
    def __init__(self):
        ...
        if self.args.side:
            self.args.dataroot = os.path.join(
                './data/{}/front'.format(self.args.gar_type))
            self.model_name = self.model_name + '_front'
            self.args.mask_location = os.path.join(
                TWO_UP, 'assets/uv_masks/{}_front.jpg'.format(self.args.gar_type))
        else:
            self.args.dataroot = os.path.join(
                './data/{}/back'.format(self.args.gar_type))
            self.model_name = self.model_name + '_back'
            self.args.mask_location = os.path.join(
                TWO_UP, 'assets/uv_masks/{}_back.jpg'.format(self.args.gar_type))
        ...
        self.criterion_percept = PercepLoss.VGGLoss(
            device=self.device, use_perceptual=True, imagenet_norm=True)

        self.optimizer_G = torch.optim.Adam(
            self.net_map.parameters(), lr=self.args.lr, betas=(0.5, 0.999))

        self.train_dataset = data.data_map(
            dataroot=self.args.dataroot, mask_location=self.args.mask_location)
        self.train_dataloader = DataLoader(
            self.train_dataset, batch_size=1, num_workers=self.args.num_workers, shuffle=True)
        self.args.iter_p_batch = len(
    ...
    def get_args(self):
        ...
        ap.add_argument("-g", "--gpu_ids", type=str,
                        default="-1", help="gpu ids, -1 for cpu")

        ap.add_argument("-w", "--num_workers", type=int,
                        default=1, help="number of workers")
        ap.add_argument("-lr", "--lr", type=float,
                        default=0.0001, help="Learning rate")
        ap.add_argument("-c", "--check", type=int, default=100,
                        help="check point after these iterations")
        ap.add_argument("-s", "--save_point", type=int,
                        default=50, help="Save models after this epoch")

        ap.add_argument("--lambda_reg", type=float,
                        default=1, help="Regresion loss")
        ap.add_argument("--lambda_recon", type=float, default=0,
                        help="Reconstruction loss at the end")
        ap.add_argument("--lambda_per", type=float, default=0,
                        help="Weight of perceptual loss")

        ap.add_argument("--gar_type", type=str,
                        default="shirts", help="shirts|pants|shorts")
        ap.add_argument("--side", type=str2bool, default=True,
                        help="Whether the front or the back is being processed")
        args = ap.parse_args()
    ...
    def backward_G(self):
        ...
        if self.args.lambda_reg:
            self.loss_reg = self.criterion_reg(
                self.uv_mask * self.out, self.uv_mask * self.cords)
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for step, sample in enumerate(self.train_dataloader):
                ...
                if (step + 1) % self.args.check == 0:
                    ...
                    log = "Elapsed [{}], Epoch [{}] Iteration [{}/{}]".format(
                        et, epoch + 1, step, self.args.iter_p_batch)
                    print(log)
            ...
            if ((epoch + 1) % self.args.save_point == 0):
                gen_file = os.path.join(
                    self.dirs['model_dir'], 'network_epoch{}.pt'.format(epoch + 1))
