...
def normalize_tensor(in_feat, eps=1e-10):
    ...
    norm_factor = torch.sqrt(torch.sum(
        in_feat**2, dim=1)).view(in_feat.size()[0], 1, in_feat.size()[2], in_feat.size()[3])
...
class VGGLoss(nn.Module):
    def __init__(self,  device, network='vgg', use_perceptual=True, imagenet_norm=False, use_style_loss=0):
        ...
        if not self.imagenet_norm:
            self.shift = torch.autograd.Variable(torch.Tensor(
                [-.030, -.088, -.188]).view(1, 3, 1, 1)).to(device)
            self.scale = torch.autograd.Variable(torch.Tensor(
                [.458, .448, .450]).view(1, 3, 1, 1)).to(device)
        ...
        if self.use_perceptual:
            ...
            self.load_state_dict(torch.load(
                os.path.join(path, network+'.pth')), strict=False)
        for param in self.parameters():
            param.requires_grad = False
    ...
    def gram2(self, x):
        ...
        for j in range(a):
            ...
            features = x[j, :, :, :].view(b, c * d)
            # compute the gram product
            G[j, :, :] = torch.mm(features, features.t())
    ...
    def forward(self, x, y):
        ...
        x, y = x.expand(x.size(0), 3, x.size(2), x.size(3)), y.expand(
            y.size(0), 3, y.size(2), y.size(3))
        ...
        loss = 0
        if self.use_perceptual:
            normed_x = [normalize_tensor(x_vgg[kk])
                        for (kk, out0) in enumerate(x_vgg)]
            normed_y = [normalize_tensor(y_vgg[kk])
                        for (kk, out0) in enumerate(y_vgg)]
            diffs = [(normed_x[kk]-normed_y[kk].detach()) **
                     2 for (kk, out0) in enumerate(x_vgg)]
            ...
            if self.use_style_loss:
                ...
                for kk in range(3, len(x_vgg)):
                    style_loss += self.criterion(
                        self.gram3(x_vgg[kk]), self.gram3(y_vgg[kk]))
