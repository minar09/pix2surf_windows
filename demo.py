...
class Demo():
    def __init__(self):
        ...
        self.net_map = network.ResnetGenerator(
            2, 2, 0, 64, n_blocks=6, norm_layer=nn.InstanceNorm2d)
        self.net_seg = network.UnetGenerator(
            input_nc=3, output_nc=2, num_downs=8, ngf=64, norm_layer=nn.InstanceNorm2d)
    ...
    def get_args(self):
        ...
        ap.add_argument("--low_type", type=str,
                        default='shorts', help='pants|shorts')

        ap.add_argument("--output", type=str, default='./output',
                        help="Location where renderings are stored")
        ap.add_argument("--video", type=str, default='./video',
                        help="location where text maps and videos are stored")

        ap.add_argument("--body_tex", type=str,
                        default='./test_data/images/body_tex/body_tex.jpg')

        self.opt = ap.parse_args()
        assert (int(self.opt.img_id) < 5 and int(self.opt.img_id)
                >= 0), 'Please enter an img_id between 0 and 4'
        assert (int(self.opt.pose_id) < 5 and int(self.opt.pose_id)
                >= 0), 'Please enter a pose_id between 0 and 4'

    def get_path(self):
        self.opt.low_mesh = './test_data/meshes/' + \
            self.opt.low_type + '/lower_{}.obj'.format(self.opt.pose_id)
        self.opt.up_mesh = './test_data/meshes/' + \
            self.opt.low_type + '/upper_{}.obj'.format(self.opt.pose_id)
        self.opt.body_mesh = './test_data/meshes/' + \
            self.opt.low_type + '/body_{}.obj'.format(self.opt.pose_id)

        self.opt.img_up_front = './test_data/images/' + \
            self.opt.low_type + '/shirt{}.jpg'.format(self.opt.img_id)
        self.opt.img_up_back = './test_data/images/' + \
            self.opt.low_type + '/shirt{}_b.jpg'.format(self.opt.img_id)

        self.opt.img_low_front = './test_data/images/' + self.opt.low_type + \
            '/{}{}.jpg'.format(self.opt.low_type, self.opt.img_id)
        self.opt.img_low_back = './test_data/images/' + self.opt.low_type + \
            '/{}{}_b.jpg'.format(self.opt.low_type, self.opt.img_id)
        ...
        self.opt.seg_low_front = './pretrained/seg_{}.pt'.format(
            self.opt.low_type)
        self.opt.seg_low_back = './pretrained/seg_{}.pt'.format(
            self.opt.low_type)

        self.opt.map_low_front = './pretrained/map_{}_front.pt'.format(
            self.opt.low_type)
        self.opt.map_low_back = './pretrained/map_{}_back.pt'.format(
            self.opt.low_type)
    ...
    def combine_textures(self):
        ...
        for val in dirs:
            ...
            # save_file = "{}/{}.jpg'".format(self.opt.output, val)
            # setattr(self.opt, 'tex_loc_' + val, save_file)
            setattr(self.opt, 'tex_loc_' + val, os.path.abspath(save_file))
    ...
    def run(self):
        ...
        os.system('blender --background --python render.py -- --body_tex {} --body_mesh {} --up_tex {} --up_mesh {} --low_mesh {} --low_tex {} --renderfolder {}'.format(
            ...
            os.path.abspath(self.opt.body_tex), os.path.abspath(self.opt.body_mesh), os.path.abspath(self.opt.tex_loc_up), os.path.abspath(self.opt.up_mesh), os.path.abspath(self.opt.low_mesh), os.path.abspath(self.opt.tex_loc_low), os.path.abspath(self.opt.video)
        ))

        self.make_video()
        # os.system('rm -r {}'.format(self.opt.output))
        os.system('rmdir /Q /S {}'.format(self.opt.output.split("/")[1]))
        # os.system('rm -r {}'.format(self.opt.video))
        os.system('rmdir /Q /S {}'.format(self.opt.video.split("/")[1]))
